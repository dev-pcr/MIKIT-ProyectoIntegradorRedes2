...
export async function processTextStream(apiKey, promptText, texts, onProgress) {
  const response = await fetch(`${API_BASE_URL}/process_text`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ api_key: apiKey, prompt_text: promptText, texts }),
  });

  if (!response.ok) {
    const error = await response.json().catch(() => ({ detail: 'Error al procesar el texto' }));
    throw new Error(error.detail || 'Error al procesar el texto');
  }
